...
WORKDIR /var/www/thesis

# install basics
RUN groupadd --gid 1000 thesis \
  && useradd --uid 1000 --gid thesis --shell /bin/bash --create-home thesis \
...
RUN curl -sL https://packages.sury.org/php/apt.gpg | apt-key add - \
  ...
  && mkdir /var/www/.composer && chown thesis:thesis /var/www/.composer && chmod 750 /var/www/.composer \
  ...
  && sed -i "s/listen = \/run\/php\/php${PHP_VERSION}-fpm.sock/listen = \/run\/php-fpm\/php-fpm.sock/" /etc/php/$PHP_VERSION/fpm/pool.d/www.conf
...
CMD chown 1000:1000 -R /var/www/thesis \
